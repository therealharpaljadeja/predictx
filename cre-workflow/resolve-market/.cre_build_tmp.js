...
var onCronTrigger = (runtime2) => {
  ...
  runtime2.log("PredictX: Checking for markets to resolve...");
  const nextIdCallData = encodeFunctionData({
    abi: MarketRegistryABI,
    functionName: "nextMarketId"
  });
  const nextIdReply = evmClient.callContract(runtime2, {
    call: encodeCallMsg({
      from: zeroAddress,
  ...
  runtime2.log("Found " + marketCount.toString() + " total markets");
  const now = Math.floor(runtime2.now().getTime() / 1000);
  let resolvedCount = 0;
  for (let i2 = 0;i2 < Number(marketCount); i2++) {
    const getMarketCallData = encodeFunctionData({
      abi: MarketRegistryABI,
      functionName: "getMarket",
      args: [BigInt(i2)]
    });
    const marketReply = evmClient.callContract(runtime2, {
      call: encodeCallMsg({
        from: zeroAddress,
        to: config.marketRegistryAddress,
    ...
    if (market.status >= STATUS_RESOLVED)
      continue;
    if (market.resolutionDate > now)
      continue;
    runtime2.log("Market #" + i2 + ": endpoint=" + market.endpointPath + ", target=" + market.targetValue.toString() + ", resolving...");
    const bearerToken = runtime2.getSecret({ id: "X_API_BEARER_TOKEN" }).result().value;
    const fetchValue = (nodeRuntime) => {
      const nodeHttpClient = new ClientCapability2;
      const resp = nodeHttpClient.sendRequest(nodeRuntime, {
    ...
    const getValue = runtime2.runInNodeMode(fetchValue, consensusMedianAggregation());
    const actualValue = getValue().result();
    runtime2.log("Fetched value for market #" + i2 + ": " + actualValue);
    const reportData = encodeAbiParameters(parseAbiParameters("uint256 marketId, uint256 actualValue"), [BigInt(i2), BigInt(actualValue)]);
    ...
    runtime2.log("Resolved market #" + i2 + " (value=" + actualValue + ") tx: " + txHash);
    resolvedCount++;
  }
  runtime2.log("Done. Found " + resolvedCount + " markets ready to resolve.");
